...
const ThankyouPageTryOut = props => {
  const navigation = useNavigation();
  const paymentData = props?.route?.params?.paymentData;
  const tryoutSetDetailData = props?.route?.params?.tryoutSetDetailData;

  const accessId = tryoutSetDetailData?.data?.access_type?.id ?? null;
  const canBuyWithToken = accessId === 1 || accessId === 3;

  console.log('paymentData', paymentData);
  console.log('tryoutSetDetailData', tryoutSetDetailData);
  return (
    <View style={styles.container}>
        ...
        <View style={[styles.row, styles.detailItemContainer]}>
          ...
          <View>
            ...
            <Text style={styles.detailItemValueText}>
              {tryoutSetDetailData.data.tryout_name}
            </Text>
          </View>
        </View>
        <View style={[styles.row, styles.detailItemContainer]}>
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          navigation.replace('StartTryOut', {
            tryoutId: tryoutSetDetailData.data.id,
          });
        }}
      >
...
export default ThankyouPageTryOut;
