...
const StartExercisesPage = props => {
  ...
  const { exercisesSetDetailData, exerciseAttemptList, exercisesSpinner } =
  ...
  useFocusEffect(
    useCallback(() => {
      const initializeData = async () => {
        ...
        const { attempts_used, no_time_limit_flag, end_time } = response.data;
      ...
      initializeData();
    }, [dispatch, exercisesSetId]),
  );

  const fetchAttempList = async () => {
    const token = await AsyncStorage.getItem('auth_token');
    dispatch(ActionStudent.GetAttemptList(token, exercisesSetId));
  };

  const handleCreateNewAttempt = async () => {
    const token = await AsyncStorage.getItem('auth_token');
    const response = await dispatch(
      ActionStudent.StartNewAttempt(token, exercisesSetId),
    );
    if (response.data) {
      navigation.navigate('StartExercisesDetailPage', {
  ...
  const RenderAttemptList = ({ data, onPressItem }) => {
    ...
    if (!submittedData.length) {
      return (
        <Text style={bottomModalStyles.emptyText}>Belum ada hasil latihan</Text>
      );
    }
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.bodyContainer}>
        <View style={styles.selectedItemContainer}>
          ...
          {exercisesSetDetailData.data.description !== '' && (
            <View style={styles.descContainer}>
              ...
              <Text style={[styles.dateText, { flexShrink: 1 }]}>
                {exercisesSetDetailData.data.description}
              </Text>
            </View>
          )}
          <View style={styles.materialDescContainer}>
            ...
            <Text style={styles.materialDateText}>
              {`Kesempatan: ${exercisesSetDetailData.data.attempts_used}/${exercisesSetDetailData.data.max_attempts}`}
            </Text>
          </View>
        </View>
        {exercisesSetDetailData.data.attempts_used > 0 && (
          <View style={styles.selectedItemContainer}>
        ...
        {!isExpired && hasAnyResource && (
          <View style={styles.resourceContainer}>
            ...
            {exercisesSetDetailData.data.guidebook_url !== '' && (
              <View style={styles.resourceChildContainer}>
                ...
                <TouchableOpacity
                  onPress={() => {
                    navigation.navigate('ExerciseGuidelinePage', {
                      pdfUrl: exercisesSetDetailData.data.guidebook_url,
                    });
                  }}
            ...
            {exercisesSetDetailData.data.telegram_group_url !== '' &&
              exercisesSetDetailData.data.whatsapp_group_url !== '' && (
                <View style={styles.resourceChildContainer}>
                  <View style={styles.resourceTitleContainer}>
                    <View style={styles.resourceIconContainer}>
                      <MaterialCommunityIcons
      ...
      <View style={styles.bottomComponent}>
        <View style={styles.buttonContainer}>
          {(exercisesSetDetailData.data.attempts_used > 0 || isExpired) && (
            <TouchableOpacity
...
export default StartExercisesPage;
