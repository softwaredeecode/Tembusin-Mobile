...
const ReviewExercisePage = props => {
  ...
  const getMyReviewResult = async () => {
    const url = `${BASE_URL}${EXERCISES.exercisesSetDetail}/${resultData.practice_set_id}/attempts/${resultData.id}/review`;
          ...
          {exerciseDetailContent?.data?.question_type?.id === 3 && (
            <View>
              ...
              <View style={[styles.contentChildContainer, { marginTop: 5 }]}>
                <Text style={styles.exercisesContentText}>
                  {exerciseDetailContent?.data?.answers[0].content}
                </Text>
              </View>
            </View>
          )}
